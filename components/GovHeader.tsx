...
export default function GovHeader() {
  ...
  return (
    <header className={`${styles.header} ${scrollDirection === 'down' ? styles.headerHidden : ''}`} style={{ background: 'linear-gradient(135deg, #1e3c72 0%, #2a5298 100%)' }}>
        ...
        <div className={styles.headerContent}>
          <div className={styles.logo}>
            <Image 
              ...
              priority
            />
            <span className={styles.logoText}>GreenGuardian</span>
          </div>
          ...
          <nav id="gov-navigation" className={`${styles.nav} ${isMenuOpen ? styles.navOpen : ''}`} role="navigation" aria-label="Government Portal">
            <ul className={styles.navList} role="menubar">
              ...
              <li role="none">
                <Link 
                  href="/gov-portal/profile" 
                  className={pathname === '/gov-portal/profile' ? styles.active : ''}
                  onClick={closeMenu}
                  role="menuitem"
                  aria-current={pathname === '/gov-portal/profile' ? 'page' : undefined}
                >
                ...
                <>
                  <li role="none" className={styles.userInfo}>
                    <div style={{ 
                      ...
                      <span style={{ fontSize: '14px' }}>{user.displayName || user.email}</span>
                    </div>
                  </li>
                  <li role="none">
                    <button 
      ...
      {isMenuOpen && (
        <nav className={styles.mobileMenu}>
          <div style={{ 
            ...
            <small style={{ color: '#666' }}>{user?.displayName || user?.email}</small>
          </div>
          ...
          <Link href="/gov-portal/profile" onClick={closeMenu}>
