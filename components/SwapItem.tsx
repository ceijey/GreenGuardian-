...
interface SwapItemProps {
  ...
  onSwapRequest: (itemId: string, offerDetails: string, offerValue: number, offerImage?: string) => void;
...
export default function SwapItem({ item, currentUser, onSwapRequest, onDelete }: SwapItemProps) {
  ...
  const handleSubmitSwapRequest = async (offerDetails: string, offerValue: number, offerImage?: string) => {
    setRequesting(true);
    try {
      await onSwapRequest(item.id, offerDetails, offerValue, offerImage);
    } finally {
      setRequesting(false);
    }
  };
